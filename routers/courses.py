...
def create_course(course: Course) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error creating course: {str(e)}")
...
def get_all_courses(
    ...
    try:
        ...
        return {"courses": courses, "total": len(courses), "status": "success"}
    except Exception as e:
        raise HTTPException(
...
def update_course(course_id: int, course_update: CourseUpdate) -> Dict[str, Any]:
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="No fields to update or update failed"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error updating course: {str(e)}")
...
def delete_course(course_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Failed to delete course")
        ...
        raise HTTPException(
            status_code=500, detail=f"Error deleting course: {str(e)}")
